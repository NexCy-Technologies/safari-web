...
const firebaseLoader = ({ src, width }: { src: string; width: number }) =>
  `${src}?w=${width}`;
...
const Gallery: React.FC<GalleryProps> = ({ className = "" }) => {
  ...
  const ImageCard = memo<{ image: GalleryImage; index: number }>(
    ({ image, index }) => (
      <div
        ...
        {!isExpanded && images.length > 8 && (
          <div className="text-center mt-8">
            <button
              ...
              className="px-6 py-3 bg-green-600 hover:bg-green-700 text-white rounded-full font-semibold transition-colors duration-300"
            >
